...
export class SpecialisationsService {
  ...
  create(
    createSpecialisationInput: CreateSpecialisationInput
  ): Promise<Specialisation> {
    const newSpecialisation = this.specialisationRepository.create(
  ...
  async remove(id: number): Promise<Specialisation> {
    const toBeDeletedSpecialisation = await this.findOneById(id);
    await this.specialisationRepository.remove(toBeDeletedSpecialisation);

    return toBeDeletedSpecialisation;
  }
}
