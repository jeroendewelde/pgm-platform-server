...
export class PersonInformationsResolver {
  constructor(private readonly personInformationsService: PersonInformationsService) {}

  @Mutation(() => PersonInformation)
  createPersonInformation(@Args('createPersonInformationInput') createPersonInformationInput: CreatePersonInformationInput): Promise<PersonInformation> {
  ...
  findOne(@Args('id', { type: () => Int }) id: number): Promise<PersonInformation> {
    return this.personInformationsService.findOneById(id);
  }

  @ResolveField(returns => [FieldExperience])
  fieldExperiences(@Parent() personInformation: PersonInformation): Promise<FieldExperience[]> {
    return this.personInformationsService.getFieldExperiences(personInformation.id);
  }

  @ResolveField(returns => [SocialMedia])
  socialMedias(@Parent() personInformation: PersonInformation): Promise<SocialMedia[]> {
  ...
  updatePersonInformation(
    ...
    return this.personInformationsService.update(id, updatePersonInformationInput);
  }

  @Mutation(() => PersonInformation)
  removePersonInformation(@Args('id', { type: () => Int }) id: number): Promise<PersonInformation> {
    const toBeDeletedPersonInformation = this.personInformationsService.findOneById(id);
