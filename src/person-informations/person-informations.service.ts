...
export class PersonInformationsService {
  ...
  create(createPersonInformationInput: CreatePersonInformationInput): Promise<PersonInformation> {
    const newPersonInformation = this.personInformationRepository.create(createPersonInformationInput);
  ...
  findOneById(id: number): Promise<PersonInformation> {
    return this.personInformationRepository.findOneOrFail(id);
  }

  findByPersonId(personId: number): Promise<PersonInformation> {
    return this.personInformationRepository.findOneOrFail({
      where: {
        personId: personId,
      }
    })
  ...
  getFieldExperiences(personInformationId: number): Promise<FieldExperience[]> {
    return this.fieldExperienceService.findByPersonInformationId(personInformationId);
  }

  getSocialMedias(personInformationId: number): Promise<SocialMedia[]> {
    return this.socialMediaService.findByPersonInformationId(personInformationId);
  }

  update(id: number, updatePersonInformationInput: UpdatePersonInformationInput): Promise<PersonInformation> {
    return this.personInformationRepository.save({
      id: id,
