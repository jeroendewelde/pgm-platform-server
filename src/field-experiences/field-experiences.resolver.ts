...
export class FieldExperiencesResolver {
  constructor(private readonly fieldExperiencesService: FieldExperiencesService) {}

  @Mutation(() => FieldExperience)
  createFieldExperience(@Args('createFieldExperienceInput') createFieldExperienceInput: CreateFieldExperienceInput): Promise<FieldExperience> {
  ...
  findOne(@Args('id', { type: () => Int }) id: number): Promise<FieldExperience> {
  ...
  @Mutation(() => FieldExperience)
  removeFieldExperience(@Args('id', { type: () => Int }) id: number): Promise<FieldExperience> {
    const toBeDeletedFieldExperience = this.fieldExperiencesService.findOneById(id);
