...
export class CoursesResolver {
  ...
  attachments(@Parent() course: Course): Promise<Attachment[]> {
  ...
  @ResolveField(() => Specialisation)
  specialisation(
    @Parent()
    course: Course
  ): Promise<Specialisation | null> {
    return this.coursesService.getSpecialisation(course.id);
  }

  @Query(() => [Course], { name: "coursesByLearningLineId" })
  findAllByLearningLineId(
    @Args("learningLineId", { type: () => Int }) learningLineId: number
  ): Promise<Course[]> {
